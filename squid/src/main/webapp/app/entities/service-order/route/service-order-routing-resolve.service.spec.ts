jest.mock('@angular/router');

import { TestBed } from '@angular/core/testing';
import { HttpResponse } from '@angular/common/http';
import { HttpClientTestingModule } from '@angular/common/http/testing';
import { ActivatedRouteSnapshot, Router } from '@angular/router';
import { of } from 'rxjs';

import { IServiceOrder, ServiceOrder } from '../service-order.model';
import { ServiceOrderService } from '../service/service-order.service';

import { ServiceOrderRoutingResolveService } from './service-order-routing-resolve.service';

describe('Service Tests', () => {
  describe('ServiceOrder routing resolve service', () => {
    let mockRouter: Router;
    let mockActivatedRouteSnapshot: ActivatedRouteSnapshot;
    let routingResolveService: ServiceOrderRoutingResolveService;
    let service: ServiceOrderService;
    let resultServiceOrder: IServiceOrder | undefined;

    beforeEach(() => {
      TestBed.configureTestingModule({
        imports: [HttpClientTestingModule],
        providers: [Router, ActivatedRouteSnapshot],
      });
      mockRouter = TestBed.inject(Router);
      mockActivatedRouteSnapshot = TestBed.inject(ActivatedRouteSnapshot);
      routingResolveService = TestBed.inject(ServiceOrderRoutingResolveService);
      service = TestBed.inject(ServiceOrderService);
      resultServiceOrder = undefined;
    });

    describe('resolve', () => {
      it('should return IServiceOrder returned by find', () => {
        // GIVEN
        service.find = jest.fn(id => of(new HttpResponse({ body: { id } })));
        mockActivatedRouteSnapshot.params = { id: 123 };

        // WHEN
        routingResolveService.resolve(mockActivatedRouteSnapshot).subscribe(result => {
          resultServiceOrder = result;
        });

        // THEN
        expect(service.find).toBeCalledWith(123);
        expect(resultServiceOrder).toEqual({ id: 123 });
      });

      it('should return new IServiceOrder if id is not provided', () => {
        // GIVEN
        service.find = jest.fn();
        mockActivatedRouteSnapshot.params = {};

        // WHEN
        routingResolveService.resolve(mockActivatedRouteSnapshot).subscribe(result => {
          resultServiceOrder = result;
        });

        // THEN
        expect(service.find).not.toBeCalled();
        expect(resultServiceOrder).toEqual(new ServiceOrder());
      });

      it('should route to 404 page if data not found in server', () => {
        // GIVEN
        jest.spyOn(service, 'find').mockReturnValue(of(new HttpResponse({ body: null as unknown as ServiceOrder })));
        mockActivatedRouteSnapshot.params = { id: 123 };

        // WHEN
        routingResolveService.resolve(mockActivatedRouteSnapshot).subscribe(result => {
          resultServiceOrder = result;
        });

        // THEN
        expect(service.find).toBeCalledWith(123);
        expect(resultServiceOrder).toEqual(undefined);
        expect(mockRouter.navigate).toHaveBeenCalledWith(['404']);
      });
    });
  });
});
