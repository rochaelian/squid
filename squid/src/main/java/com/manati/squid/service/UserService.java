...
import com.manati.squid.repository.AuthorityRepository;
...
public class UserService {
    ...
    public User registerUser(AdminUserDTO userDTO, String password) {
        ...
        if (userDTO.getAuthorities() != null) {
            ...
            if (userDTO.getAuthorities().contains("ROLE_ADMIN") || userDTO.getAuthorities().contains("ROLE_USER")) {
                authorities.clear();
        ...
        SecurityUtils
            ...
            .ifPresent(
                user -> {
                    ...
                    user.setPassword(encryptedPassword);
                    Optional<UserDetails> userDetails = userDetailsRepository.findByInternalUser_Id(user.getId());
                    if (userDetails.isPresent()) {
                        UserDetails operator = userDetails.get();
                        Status pending = Status.Pending;
                        if (operator.getStatus().equals(pending)) {
                            Status operatorStatus = Status.Enabled;
                            operator.setStatus(operatorStatus);
                            userDetailsRepository.save(operator);
                        }
                    }
                    this.clearUserCaches(user);
